...
class Iluminacion extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private static String TITLE = "APLICACION DE ILUMINACION BALON DE FUTBOL";  // window's title
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        glut = new GLUT();
        this.textura = this.cargarTextura("imagenes/pelota.jpg");
    }
...
//        gl.glMaterialfv( face, GL.GL_EMISSION, colorBlack , 0 );
    ...
    // Añade esta línea para habilitar el mapeo de texturas
    //gl.glEnable(GL2.GL_TEXTURE_2D);

    // Colocar la esfera en el centro
    gl.glTranslatef(0.0f, 0.0f, 0.0f);

    // Establecer el material y dibujar la esfera
    setSomeWhiteMaterial(gl, GL.GL_FRONT);

    // Ajusta el número de divisiones para stacks y slices según tus necesidades
    int stacks = 20;
    int slices = 20;
    float radius = 1.0f;
    ...
    float thetaDelta = (float) (Math.PI / stacks);
    float phiDelta = (float) (2.0 * Math.PI / slices);

    for (int j = 0; j < stacks - 1; j++) {
        gl.glBegin(GL2.GL_QUAD_STRIP);
        for (int i = 0; i < slices; i++) {
            float s = (float) i / (float) slices;
            float t = (float) j / (float) (stacks - 1);

            gl.glTexCoord2f(s, t);
            gl.glVertex3f((float) (radius * Math.sin(theta1) * Math.cos(phi)), (float) (radius * Math.cos(theta1)), (float) (radius * Math.sin(theta1) * Math.sin(phi)));

            s = (float) (i + 1) / (float) slices;
            gl.glTexCoord2f(s, t);
            gl.glVertex3f((float) (radius * Math.sin(theta2) * Math.cos(phi)), (float) (radius * Math.cos(theta2)), (float) (radius * Math.sin(theta2) * Math.sin(phi)));
